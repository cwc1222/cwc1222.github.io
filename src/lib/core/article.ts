...
class ArticleManager {
  ...
  private async loadArticles() {
    ...
    this.#articles = (await articlesLst).sort((a, b) => { // Desc
      const d1 = a.attributes.updatedAt ? new Date(a.attributes.updatedAt) : new Date(a.attributes.createdAt);
      const d2 = b.attributes.updatedAt ? new Date(b.attributes.updatedAt) : new Date(b.attributes.createdAt);
      if (d1 > d2) {
        return -1;
      }
      if (d1 < d2) {
