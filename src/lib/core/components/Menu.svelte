...
<div class="d-md-none d-flex align-items-center justify-content-between mb-3">
	<button class="btn" on:click={toggleMenuEvt}>
...
<div class="offcanvas-md offcanvas-start menu" tabindex="-1" bind:this={offcanvas}>
	<div class="offcanvas-header">
		<button type="button" class="btn-close" aria-label="Close" on:click={toggleMenuEvt}> </button>
	</div>
	<div class="offcanvas-body">
		<nav class="container-fluid" style="min-height: 85vh;">
			<div
				class="row d-flex flex-column align-items-center justify-content-between text-center gap-2 fs-5 h-100"
			>
				<div>
					<img
						src={menuInfo.avatar}
						...
						<ul class="m-0 p-0">
							{#each menuInfo.blogPages as p}
								<li class="list-group-item">
									<a href={p.href} class="list-group-item list-group-item-action">{p.name}</a>
								</li>
							{/each}
						</ul>
					</div>
				...
				<div class="d-md-block d-none">
					<div>
						{#each menuInfo.socialMedias as sm}
							<a href={sm.url} title={sm.name} class="{sm.class} px-1" target="_blank"
								>{@html sm.icon}</a
							>
						{/each}
					</div>
