...
for await (const file of glob.scan('.')) {
	...
	articles = [
		...articles,
		{
			slug: splitedPath[splitedPath.length - 2],
...
articles.sort((a, b) => {
	// Desc
	const d1 = a.attributes.updatedAt ? a.attributes.updatedAt : a.attributes.createdAt;
	const d2 = b.attributes.updatedAt ? b.attributes.updatedAt : b.attributes.createdAt;
	if (d1 > d2) {
		return -1;
	}
	if (d1 < d2) {
